...
# This source file was contributed by Jitendra Verma in Meshes.jl
# and adapted to CoordRefSystems.jl with permission under the same
# shared MIT License.

"""
    geodesicfwd(coords, ϕ, l)

Solve the direct geodesic problem: return the coordinates reached from
the coordinates `coords` after walking the length `l` along the geodesic that
leaves `coords` with the azimuth `ϕ`, measured clockwise from the north.

See also [`geodesicbwd`](@ref), [`geodesicdistance`](@ref).

## Examples

```julia
geodesicfwd(LatLon(0, 0), 90u"°", 1000u"km")
```
...
"""
function geodesicfwd(coords::LatLon{Datum}, ϕ, l) where {Datum}
  ...
  lat = S(ustrip(coords.lat))
  lon = S(ustrip(coords.lon))
  azi = S(ustrip(ϕ′))
  len = S(ustrip(u, l′))

  lat′, lon′, _ = _geodesicdirect(🌎, lat, lon, azi, len)

  LatLon{Datum}(R(lat′), R(lon′))
end

# fallback for other coordinate reference systems
function geodesicfwd(coords::C, ϕ, l) where {C<:CRS}
  latlon₁ = convert(LatLon, coords)
  latlon₂ = geodesicfwd(latlon₁, ϕ, l)
  convert(C, latlon₂)
end
...
See also [`geodesicfwd`](@ref), [`geodesicdistance`](@ref).

## Examples

```julia
geodesicbwd(LatLon(0, 0), LatLon(0, 1))
```
...
* Karney, C. F. F. 2013. [Algorithms for geodesics](https://doi.org/10.1007/s00190-012-0578-z)
"""
function geodesicbwd(coords₁::LatLon, coords₂::LatLon)
  # convert coordinates to same LatLon CRS
  coords₁′, coords₂′ = promote(coords₁, coords₂)

  # ellipsoid of revolution
  🌎 = ellipsoid(datum(coords₁′))

  # the series of Karney need double precision to reach round-off
  T = numtype(lentype(coords₁′))
  S = promote_type(T, Float64)
  lat₁, lon₁ = S(ustrip(coords₁′.lat)), S(ustrip(coords₁′.lon))
  lat₂, lon₂ = S(ustrip(coords₂′.lat)), S(ustrip(coords₂′.lon))
...
# fallback for other coordinate reference systems
function geodesicbwd(coords₁::CRS, coords₂::CRS)
  latlon₁ = convert(LatLon, coords₁)
  latlon₂ = convert(LatLon, coords₂)
  geodesicbwd(latlon₁, latlon₂)
end

"""
    geodesicdistance(coords₁, coords₂)

Compute the geodesic distance between two coordinates
`coords₁` and `coords₂` on the ellipsoid.

## Examples

```julia
geodesicdistance(LatLon(0, 0), LatLon(0, 1))
```
...
* Karney, C. F. F. 2013. [Algorithms for geodesics](https://doi.org/10.1007/s00190-012-0578-z)
"""
function geodesicdistance(coords₁::LatLon, coords₂::LatLon)
  # convert coordinates to same LatLon CRS
  coords₁′, coords₂′ = promote(coords₁, coords₂)
...
# fallback for other coordinate reference systems
function geodesicdistance(coords₁::CRS, coords₂::CRS)
  latlon₁ = convert(LatLon, coords₁)
  latlon₂ = convert(LatLon, coords₂)
  geodesicdistance(latlon₁, latlon₂)
end

"""
    geodesictangent(coords, ϕ)

Unit vector tangent to the ellipsoid at the coordinates `coords`,
...
```julia
geodesictangent(LatLon(0, 0), 90u"°")
```
"""
...
# fallback for other coordinate reference systems
geodesictangent(coords::CRS, ϕ) = geodesictangent(convert(LatLon, coords), ϕ)

"""
    geodesicazimuth(coords, v)

Azimuth of the vector `v` at the coordinates `coords` of the ellipsoid,
measured clockwise from the north. Any component of `v` along the normal
of the ellipsoid is ignored, and the azimuth is undefined if nothing is left.
...
```julia
geodesicazimuth(LatLon(0, 0), (0, 1, 0))
```
"""
...
# fallback for other coordinate reference systems
geodesicazimuth(coords::CRS, v) = geodesicazimuth(convert(LatLon, coords), v)
...
# East and north unit vectors of the local frame at the coordinates, expressed
# in geocentric Cartesian coordinates. Together with the normal of the ellipsoid
# they form the frame in which azimuths are measured.
function _eastnorth(coords::LatLon)
  sφ, cφ = sincosd(ustrip(coords.lat))
  sλ, cλ = sincosd(ustrip(coords.lon))
  ê = SVector(-sλ, cλ, zero(sλ))
  n̂ = SVector(-sφ * cλ, -sφ * sλ, cφ)
  (ê, n̂)
end
