@testset "CRS API" begin
  ...
  @testset "raw" begin
    ...
    c = Polar(T(1) * u"km", T(2) * rad)
    @test CoordRefSystems.raw(c) == (T(1), T(2))
    c = Cylindrical(T(1) * u"cm", T(2) * rad, T(3) * u"cm")
    @test CoordRefSystems.raw(c) == (T(1), T(2), T(3))
    c = Spherical(T(1) * u"mm", T(2) * rad, T(3) * rad)
    @test CoordRefSystems.raw(c) == (T(1), T(2), T(3))
    c = LatLon(T(30), T(60))
  ...
  @testset "units" begin
    ...
    c = Polar(T(1) * u"km", T(1) * rad)
    @test CoordRefSystems.units(c) == (u"km", rad)
    c = Cylindrical(T(1) * u"cm", T(1) * rad, T(1) * u"cm")
    @test CoordRefSystems.units(c) == (u"cm", rad, u"cm")
    c = Spherical(T(1) * u"mm", T(1) * rad, T(1) * rad)
    @test CoordRefSystems.units(c) == (u"mm", rad, rad)
    c = LatLon(T(30), T(60))
